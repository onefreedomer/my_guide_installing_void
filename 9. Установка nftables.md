Установка nftables:
```bash
sudo xbps-install nftables runit-nftables iptables-nft
```
...
Для совместимости с приложениями, который работают только с iptables:
```bash
sudo xbps-alternatives -g iptables -s iptables-nft
```

Создание файла конфигурации брандмауэра ([пример взят с Arch Wiki](https://wiki.archlinux.org/title/Nftables)):
```bash
...
Перезапуск nftables для применения файла конфигурации:
```bash
sudo sv restart nftables
```

Проверка работает ли nftables:
```bash
sudo sv status nftables
sudo nft list ruleset
```

![Статус nftables](nftables_status.png)