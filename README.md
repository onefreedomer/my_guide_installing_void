...
## Введение
По ходу чтения руководства в начале каждой команды будут встречаться символы `$` и `#`. Первый из них обозначает, что команда выполняется от обычного пользователя, а второй - от `root` или с использованием `sudo`. Символы `$`, `#` не нужно копировать в консоль.   
...
#### Настройка отображаемого времени и установка chrony
Выберите часовой пояс:
```console
# ln -sf /usr/share/zoneinfo/<регион>/<город> /etc/localtime
```

При dualboot с Windows может происходит конфликт в отображении времени, так как Windows по умолчанию использует локальное время, а большинство дистрибутивов Linux - UTC.  
Для перехода на UTC в Windows откройте `regedit` и создайте параметр `DWORD` со значением 1 в:
```
HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\TimeZoneInformation\RealTimeIsUniversal
```

Установите `chrony` для синхронизации времени, используя интернет соединение, и добавьте его в автозагрузку:
```console
# xbps-install chrony
# ln -s /etc/sv/chronyd /var/service
```

Перезагрузите систему и проверьте время:
```console
# reboot
$ date
```

#### Включение nonfree и multilib репозиториев
Репозитории ниже нужны для установки драйверов NVIDIA и Intel, а также Steam:
```console
...
3. https://wiki.archlinux.org/title/System_time
4. https://docs.voidlinux.org/xbps/repositories/index.html